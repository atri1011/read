...
const DEFAULT_PREFS: ReaderPrefs = { fontSize: 17, measure: "normal" };
...
export function ReaderShell({
  ...
  async function handleDictAddNote(payload: {
    ...
    if (payload.anchor) {
      // Create note annotation directly from dictionary result
      const created = await createAnnotation(documentId, {
        type: "note",
        color: "yellow",
        body: payload.body,
    ...
    // No anchor — surface body into toolbar note field if possible
    ...
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="flex flex-wrap items-center gap-2 border-b border-zinc-200 bg-white/70 px-4 py-2 text-xs dark:border-zinc-800 dark:bg-zinc-950/70">
        ...
        <button
          type="button"
          aria-label="减小字号"
          className="rounded border border-zinc-200 px-2 py-0.5 hover:bg-zinc-50 disabled:opacity-40 dark:border-zinc-700 dark:hover:bg-zinc-900"
